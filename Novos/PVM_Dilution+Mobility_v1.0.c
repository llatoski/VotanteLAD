    ...
    if(spin[neighbour]!=0){
      if(spin[site]!=spin[neighbour]) {
        if(zealot[site] == 0){
          ...
          int INTERFDEPOIS=0;
          if(spin[up[site]]==-spin[site])INTERFDEPOIS++;
          if(spin[right[site]]==-spin[site])INTERFDEPOIS++;
          if(spin[down[site]]==-spin[site])INTERFDEPOIS++;
          if(spin[left[site]]==-spin[site])INTERFDEPOIS++;
...
void openfiles(void) {
  ...
  char teste[250];
  if(DETA>0){
    sprintf(root_name,"dilution_lg%d_inc1E%d_rho%.2f_mob%.2f",L,(int)log10(DETA),RHO,MOB);
  }
  else{
    sprintf(root_name,"dilution_lg%d_incNULL_rho%.2f_mob%.2f",L,RHO,MOB);
  }
  ...
  fprintf(fp1,"# Generated with: VM_Dilution+Mobility_v1.0\n");
  fprintf(fp1,"# Seed: %ld\n",seed);
  fprintf(fp1,"# Linear size: %d\n",L);
  fprintf(fp1,"# Incremento: %.6f\n",DETA);
  fprintf(fp1,"# Mobilidaade: %.1f\n",MOB);
  fflush(fp1);
