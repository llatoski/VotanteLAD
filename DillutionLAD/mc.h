...
* it is the numbers of seconds ellapsed since ?/?/1970.         *
****************************************************************/ 
void start_randomic(unsigned long semente)
...
*********************************************************************/
void create_time_table_decades(double *t1, double first_time, int decades)

{
int i,j,k=0;

i = first_time;
// t1[0] = pow(10,first_time);
while (i <= decades)
      {
       t1[k] =  pow(10,i);
       ++k;
...
int uniq(unsigned long *a, int len)

{
int i, j;

qsort(a, len, sizeof(unsigned long), icmp);
for (i = j = 0; i < len; i++)
    if (a[i] != a[j]) a[++j] = a[i];
return j + 1;
}
